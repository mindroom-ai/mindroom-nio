...
def _planned_work(
    ...
    return PlannedWork(value, plaintext, ordinal)
    ...
    *,
    ...
    for room_id, room in room_plans.items():
        ...
        if (hydration := room.hydration) is None:
            ...
            ready_before = room.before
            ready_after = room.after
            if (
                len(room_plans) != 1
                or aggregate is None
                or aggregate.pending_hydration is not None
                or ready_before is None
                or ready_before != aggregate.continuity
                or ready_before.baseline is None
                or ready_after.baseline is None
                or ready_after.baseline.membership_event_id is None
                or ready_after == ready_before
                or ready_after != replace(ready_before, baseline=ready_after.baseline)
                or ready_before.gap is not None
                or ready_after.gap is not None
                or ready_before.hydration_id is not None
                or ready_after.hydration_id is not None
    ...
    held_count = 0
    held_bytes = 0
    retired_work: list[AuthenticatedWork] = []
    ...
    for item in work:
        ...
        if held_room_id in {retirement_room_id, capacity_room_id}:
            retired_work.append(item)

    inserts: list[PlannedWork] = []
    releases: list[PlannedWork] = []

    def retired_key(item: AuthenticatedWork) -> tuple[int, str]:
        retired_value = item.value
        if type(retired_value) is not EventRecord:
            raise ValueError("retired HELD Work is not an event")
        return retired_value.room_sequence or 0, retired_value.record_id

    def retired_releases(start: int) -> list[PlannedWork]:
        planned: list[PlannedWork] = []
        for retired_ordinal, retired_item in enumerate(
            sorted(retired_work, key=retired_key),
            start,
        ):
            retired_value = retired_item.value
            if type(retired_value) is not EventRecord:
                raise ValueError("retired HELD Work is not an event")
            planned.append(
                PlannedWork(
                    retired_value,
                    retired_item.plaintext
                    or _canonical_work_plaintext("event", retired_value),
                    retired_ordinal,
                )
            )
        return planned
    ...
    if retirement is not None:
        retained = [
            item
            for item in inserts
            if _work_id(item.value) not in retirement_successor_ids
        ]
    ...
    if (
        ...
        for index, (value, plaintext, ordinal) in enumerate(retained):
            ...
            inserts.append(PlannedWork(value, plaintext, ordinal))
        ...
        releases = retired_releases(1)
        inserts = [loss_work] + [
            PlannedWork(value, plaintext, index + len(releases) + 1)
            for index, (value, plaintext, _) in enumerate(
                item for item in inserts if item.value.room_id is None
            )
        ]
    ...
    release_delta = 0
    for legacy_release in releases:
        release_value = legacy_release.value
        if type(release_value) is not EventRecord:
            raise ValueError("released HELD Work is not an event")
        release_ordinal = legacy_release.ready_ordinal
        if release_ordinal is None:
            raise ValueError("released HELD Work has no READY ordinal")
        existing_release = work_by_id[release_value.record_id]
        if existing_release.metadata is not None and (
            existing_release.frame_id is None
            or existing_release.created_revision is None
        ):
            raise ValueError("prepared HELD release lacks authenticated row identity")
        if (
            ...
            size = (
                work_sizes[release_value.record_id]
                + len(str(revision))
                + len(str(release_ordinal))
                - 7
            )
        if size > hard.max_record_canonical_bytes:
            raise ValueError("released Work record exceeds the canonical byte limit")
        release_delta += size - work_sizes[release_value.record_id]
