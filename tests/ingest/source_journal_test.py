...
def test_sliding_frame_envelope_round_trips_list_evidence_exactly(
    tmp_path: Path,
) -> None:
    source_config = SlidingSourceConfig(
        timeout_ms=30_000,
        connection_name="worker",
        lists_json=b'{"probe":{"ranges":[[0,0]]}}',
        room_subscriptions_json=b"{}",
    ...
    adapter = SlidingSource(owner.stream_id, source_config, OWN_USER_ID)
    ...
    staged = StagedFrame(
        expected.frame_id,
        StagedSourceResponse(request, normalized.response_body, expected.source_sha256),
    )
    committed = journal.stage_source_response(
        source=SourceState(
            prior.source_epoch,
            prior.transport_kind,
            expected.candidate_cursor_json,
    ...
    reopened = _open(
        tmp_path,
        SlidingSourceConfig(90_000, "drifted", b"{}", b"{}", b"{}", 99),
    )
    try:
        loaded = reopened._journal.load_frame(expected.frame_id)
        assert loaded == replace(staged, staged_revision=committed.revision)
        assert loaded is not None
        replayed = renormalize_staged_frame(
            SlidingSource(owner.stream_id, source_config, OWN_USER_ID),
            loaded,
        )
        assert replayed.sliding_list_results == expected.sliding_list_results
        assert replayed.sliding_request_config_sha256 == (
            expected.sliding_request_config_sha256
        )
        assert replayed.candidate_cursor_json == expected.candidate_cursor_json
        assert replayed == expected

        envelope = _stored_plaintext_frame_value(
            _StoredStage(
                tmp_path / "journal.db",
                owner.stream_id,
                _StageProposal(
        ...
        assert "sliding_list_results" not in envelope
        assert "candidate_cursor_json" not in envelope
        assert "sliding_request_config_sha256" not in envelope
    finally:
        reopened.close()
