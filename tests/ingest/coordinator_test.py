...
@pytest.mark.parametrize(
    ("mutation", "source_count"),
    (
        ("live_bool", 1),
        ("live_and_delta", 0),
        ("negative_without_source", None),
    ),
)
@pytest.mark.asyncio
async def test_owned_materializer_binds_uploaded_count_to_exact_source_integer(
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
    ...
    try:
        body, sender_store = _owned_crypto_classic_body(client)
        body["device_one_time_keys_count"] = (
            {} if source_count is None else {"signed_curve25519": source_count}
        )
        stage_classic(bootstrap, body)
        database_before = tuple(connection.iterdump())
        assert client.olm is not None
        ...
        def forge_uploaded_count(*args: object, **kwargs: object):
            prepared = real_prepare(*args, **kwargs)
            if mutation == "live_bool":
                olm.uploaded_key_count = True
                return prepared
            if mutation == "live_and_delta":
                olm.uploaded_key_count = 1
                return prepared._replace(
                    crypto_delta=prepared.crypto_delta._replace(uploaded_key_count=1)
                )
            if mutation == "negative_without_source":
                olm.uploaded_key_count = -1
                return prepared._replace(
                    crypto_delta=prepared.crypto_delta._replace(uploaded_key_count=-1)
                )
        ...
        client._prepare_ingestion_frame = forge_uploaded_count
        share_count = 0
        real_share_keys = olm.share_keys
        ...
        def capture_share_keys():
            ...
            share_count += 1
            return real_share_keys()

        monkeypatch.setattr(olm, "share_keys", capture_share_keys)
        statements: list[str] = []
        connection.set_trace_callback(statements.append)
        with pytest.raises(JournalIntegrityError, match="uploaded.*count"):
            session._materialize_oldest_frame(limits=MaterializerLimits())
        connection.set_trace_callback(None)
...
@pytest.mark.parametrize(
    ...
    (
        ("omitted_encrypted_rooms", "source encryption"),
        ("foreign_encrypted_room", "source encryption"),
        ("unsorted_encrypted_rooms", "source encryption"),
        ("query_live", "crypto delta"),
        ("one_time_key_source", "crypto delta"),
        ("fallback_source", "crypto delta"),
        ("claim_live", "crypto delta"),
        ("queued_live", "crypto delta"),
    ),
)
@pytest.mark.asyncio
async def test_owned_materializer_rejects_crypto_delta_that_disagrees_with_live_state(
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
    error_pattern: str,
    ...
    try:
        ...
        second_room = "!another:example.org"
        joined[second_room] = {
            "state": {
                "events": [
                    {
                        "content": {"algorithm": "m.megolm.v1.aes-sha2"},
                        "event_id": "$second-encryption",
                        "origin_server_ts": 4,
                        "sender": ACCOUNT,
                        "state_key": "",
        ...
        def forge_crypto_delta(*args: object, **kwargs: object):
            prepared = real_prepare(*args, **kwargs)
            delta = prepared.crypto_delta
            assert delta.encrypted_room_ids == (second_room, ROOM)
            if mutation == "omitted_encrypted_rooms":
                delta = delta._replace(encrypted_room_ids=())
            elif mutation == "foreign_encrypted_room":
                delta = delta._replace(encrypted_room_ids=("!foreign:example.org",))
            elif mutation == "unsorted_encrypted_rooms":
                delta = delta._replace(
                    encrypted_room_ids=tuple(reversed(delta.encrypted_room_ids))
                )
            elif mutation == "query_live":
                olm.users_for_key_query.add("@forged:example.org")
            elif mutation == "one_time_key_source":
                delta = delta._replace(
                    one_time_key_counts_json=canonical_json({"signed_curve25519": 1})
                )
            elif mutation == "fallback_source":
                delta = delta._replace(
                    unused_fallback_key_types_json=canonical_json(["signed_curve25519"])
                )
            elif mutation == "claim_live":
                olm.wedged_devices.clear()
            elif mutation == "queued_live":
                olm.outgoing_to_device_messages.append(
                    ToDeviceMessage(
                        "org.example.extra",
                        "@extra:example.org",
                ...
                raise AssertionError(f"unknown mutation {mutation}")
            return prepared._replace(crypto_delta=delta)

        client._prepare_ingestion_frame = forge_crypto_delta
        share_count = 0
        real_share_keys = olm.share_keys
        ...
        statements: list[str] = []
        connection.set_trace_callback(statements.append)
        with pytest.raises(JournalIntegrityError, match=error_pattern):
            session._materialize_oldest_frame(limits=MaterializerLimits())
        connection.set_trace_callback(None)
...
async def test_owned_runner_uses_private_materializer_in_fifo_order(
    ...
    def forbid_legacy_materializer(*_args: object, **_kwargs: object) -> None:
        nonlocal legacy_calls
        legacy_calls += 1
        raise AssertionError("owned runner invoked legacy materialization")
